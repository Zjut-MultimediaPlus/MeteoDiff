...
class DifUVChangeTC(Module):
    def __init__(self,num_features,x_dim):
        super(DifUVChangeTC, self).__init__()
        # 定义线性层用于将 [B, T2, 128] -> [B, T2, 64]
        self.fc1 = nn.Linear(num_features, x_dim)
        self.fc2 = nn.Linear(num_features, x_dim)
        self.fc3 = nn.Linear(384, 64)
        # 其中，dim_in 是输入的维度，dim_out 是输出的维度，dim_ctx 是上下文的维度。
        self.changetc1 = ConcatSquashLinear(x_dim, x_dim//2, x_dim*2)
        self.changetc2 = ConcatSquashLinear(x_dim//2, x_dim // 8, x_dim * 2)
    def forward(self, dif_v, dif_d, track_intensity):
        B, T2, C, H, W = dif_v.shape
        # 将风速和风向展平，组合成每个时间步的特征 [B, T2, C * H * W]
        wind_speed_flat = self.fc1(dif_v.view(B, T2, -1)) #(B,t2,128) #[B,t2,128]
        wind_direction_flat = self.fc2(dif_d.view(B, T2, -1)) #(B,t2,128)

        # 风场是差值 但轨迹不是差值 track_diff = track_intensity[:, 1:, :] - track_intensity[:, :-1, :]
        va_di = torch.cat([wind_speed_flat, wind_direction_flat], dim=-1) # [B, T2, 256]
        x2 = self.changetc1(va_di, track_intensity)  #[B,t2,64]
        x3 = self.changetc2(va_di, x2) #[B,T2,4]
        return x3
