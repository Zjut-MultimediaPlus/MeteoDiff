...
class Trajectron(object):
    ...
    def __init__(self,
                 model_registrar, #self.registrar=ModelRegistrar(self.model_dir, "cuda") 加载模型
                 hyperparams,
                 device):
        ...
        self.device = device
        self.curr_iter = 0 #curr_iter: 当前迭代次数

        self.model_registrar = model_registrar
        #通过 set_environment 方法设置为具体的节点模型。
        self.node_models_dict = dict() #node_models_dict: 存储节点模型的字典，每个节点模型对应一个节点类型。
        self.nodes = set() #nodes: 存储模型中存在的节点集合。

        self.env = None

        self.min_ht = self.hyperparams['minimum_history_length'] #1
        self.max_ht = self.hyperparams['maximum_history_length'] #7
        self.ph = self.hyperparams['prediction_horizon'] #ph: 预测时间步的长度
        self.state = self.hyperparams['state'] #模型有的几个输入 位置 速度 加速度 6个值
        # 'state':  # 定义了状态的属性
        ...
        #       }
        #  },
        self.state_length = dict() #存储不同状态类型的长度信息
        ...
        self.pred_state = self.hyperparams['pred_state'] #存储预测状态信息

    # 设置场景中的节点模型和边缘信息
    # 总结：set_environment 方法用于设置环境，
    # 创建节点模型，并将节点模型存储在 node_models_dict 字典中，以便在后续的轨迹预测中使用。
    # 每个节点类型对应一个 MultimodalGenerativeCVAE 模型，这些模型将在轨迹预测过程中使用。
    def set_environment(self, env):
        self.env = env #所谓的env 不是“环境” 而是数据集 包括轨迹 强度 风速 历史值和预测的真值 scene.node

        self.node_models_dict.clear()
        edge_types = env.get_edge_types()  #return list(product(self.NodeType, repeat=2)) 估计也是行人

        for node_type in env.NodeType:
            # Only add a Model for NodeTypes we want to predict
            if node_type in self.pred_state.keys():
                #用于多模态生成的条件变分自编码器模型
                #将新创建的模型添加到 self.node_models_dict 字典中，
                # 使用节点类型作为键，将模型对象作为值。
...
# *******z = self.encoder.get_latent(batch, node_type)
    def get_latent(self, batch, node_type):
        (first_history_index,
         ...
         neighbors_data_st, # 基本没有 因为是邻居节点
         ...
         , vo_cen_x, vo_dis_x
         ) = batch #从输入批处理中提取必要的数据，包括历史索引、输入位置(x_t)、目标位置(y_t)、？？？
        # 输入速度(x_st_t)、目标速度(y_st_t)、邻居数据(neighbors_data_st)、
        # 邻居边缘值(neighbors_edge_value)、机器人轨迹(robot_traj_st_t)和地图（如果有）
        ...
        gph_all_hpa_data_x_norm_t = gph_all_hpa_data_x_norm_t.to(self.device)
        # gph_data_y = gph_data_y.to(self.device)

        # gph_data_x_t = gph_data_x_t.to(self.device)

        # env_data_x = env_data_x.to(self.device)
        # env_data_y = env_data_y.to(self.device)
        ...
        model = self.node_models_dict[node_type] #根据给定的节点类型获取相应的节点模型。
        #这里接受的是12的长度 所以不能分开写
        #
        #  , q_data_x_norm, t_data_x_norm
        #  )

        x_gph,encoded_age,encoded_env_data = model.get_latent(inputs=x, #torch.Size([256, 8, 12])
                                inputs_st=x_st_t,
                                ...
                                prediction_horizon=self.ph
                                  ...
                                  , timestep_range_x=timestep_range_x, timestep_range_y=timestep_range_y
                                , gph_all_hpa_data_x=gph_all_hpa_data_x, gph_all_hpa_data_y=gph_all_hpa_data_y
                                ,gph_all_hpa_data_x_norm_t=gph_all_hpa_data_x_norm_t
                                , wind_data_x=wind_data_x, wind_data_y=wind_data_y
                                , wind_data_x_norm=wind_data_x_norm, wind_data_y_norm=wind_data_y_norm
                                , q_data_x_norm=q_data_x_norm, t_data_x_norm=t_data_x_norm
                                , vo_cen_x = vo_cen_x, vo_dis_x = vo_dis_x
                                  )

        return x_gph,encoded_age,encoded_env_data#返回从节点模型获得的潜在表示（feat_x）。
    ...
    # 预测轨迹的方法。给定场景、时间步长等信息，它调用对应的节点模型来进行轨迹预测，并返回预测结果。
    def predict(self,
                scene,
                timesteps,
                ph,
