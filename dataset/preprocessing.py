...
def get_gph_500_norm(gph_500):
    gph_data_x_norm = gph_500
    rel_state_gph = gph_data_x_norm[-1]  # 全0
    mean_gph = rel_state_gph
    std_gph = np.zeros_like(gph_data_x_norm[0])
    std_gph = std_gph + 3  # 原为3
    gph_data_x_norm_t = np.where(np.isnan(gph_data_x_norm), np.array(np.nan),
                                 (gph_data_x_norm - mean_gph) / std_gph)  # -90，90
    gph_data_x_norm_t = gph_data_x_norm_t * 0.01
    return gph_data_x_norm_t


def get_gph_200_500_norm(gph_200_500): #gph_200_500: array 8,4,101,101
...
def get_wind_norm(gph_200_500):
    gph_data_x_norm = gph_200_500
    rel_state_gph = gph_data_x_norm[-1]  # 全0
    mean_gph = rel_state_gph
    std_gph = np.zeros_like(gph_data_x_norm[0])
    std_gph = std_gph + 3  # 原为3
...
def get_node_timestep_data(env, scene, t, node, state, pred_state,
    ...
    # gph_data_x_norm_t = get_gph_500_norm(gph_data_x)

    gph_all_hpa_data_x = node.get(timestep_range_x, 'gph_all_hpa_data')
    gph_all_hpa_data_y = node.get(timestep_range_y, 'gph_all_hpa_data')
    gph_all_hpa_data_x_norm_t = get_gph_200_500_norm(gph_all_hpa_data_x)
    gph_all_hpa_data_x_norm_t = gph_all_hpa_data_x_norm_t * 0.01

    # gph_200_data = node.get(timestep_range_x, 'gph_all_hpa_data')
    # # gph_500_data = node.get(timestep_range_x, 'gph_500_data')
    #
    # gph_all_hpa_data_x = np.concatenate((gph_200_data, gph_500_data[:,np.newaxis, :,:]), axis=1)
    # gph_all_hpa_data_x_norm_t = get_gph_200_500_norm(gph_all_hpa_data_x)
    #
    # gph_200_data_y = node.get(timestep_range_y, 'gph_all_hpa_data')
    # gph_500_data_y = node.get(timestep_range_y, 'gph_500_data')
    #
    # gph_all_hpa_data_y = np.concatenate((gph_200_data_y, gph_500_data_y[:,np.newaxis,:,:]), axis=1)
    ...
    wind_data_x = node.get(timestep_range_x, 'wind_data')
    wind_data_y = node.get(timestep_range_y, 'wind_data')

    wind_data_x_norm = get_wind_norm(wind_data_x)
    wind_data_y_norm = get_wind_norm(wind_data_y)

    env_data_x = node.get(timestep_range_x, 'env_data')
    env_data_y = node.get(timestep_range_y, 'env_data')

    q_data_x = node.get(timestep_range_x, 'q_data')
    # q_data_y = node.get(timestep_range_y, 'q_data')

    t_data_x = node.get(timestep_range_x, 't_data')

    q_data_x_norm = get_gph_200_500_norm(q_data_x)
    q_data_x_norm = q_data_x_norm*1000
    t_data_x_norm = get_gph_200_500_norm(t_data_x)

    vo_cen_x = node.get(timestep_range_x, 'vo_cen')
    vo_dis_x = node.get(timestep_range_x, 'vo_dis')


    # sst_data_x = node.get(timestep_range_x, 'sst_data')
    # sst_data_y = node.get(timestep_range_y, 'sst_data')

    # tcwv_data_x = node.get(timestep_range_x, 'tcwv_data')
    # tcwv_data_y = node.get(timestep_range_y, 'tcwv_data')
    #
    # temperature_data_x = node.get(timestep_range_x, 'temperature_data')
    # temperature_data_y = node.get(timestep_range_y, 'temperature_data')
