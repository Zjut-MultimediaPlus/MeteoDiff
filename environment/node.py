...
import torch
from dataset.dataset import NumpyDataset
# from torch.utils.data import Dataset, DataLoader


class Node(object):
    def __init__(self, node_type, node_id, data,
                 # gph_data,
                 env_data,
                 gph_all_hpa_data,
                 # gph_200_all_data,
                 ...
                 length=None, width=None, height=None, first_timestep=0,
        ...
        # self.gph_data = gph_data
        self.env_data = env_data
        self.gph_all_hpa_data = gph_all_hpa_data
        # self.gph_500_data = gph_500_data
        # self.gph_all_hpa_data = gph_all_hpa_data
        # self.sst_data = sst_data
        # self.tcwv_data = tcwv_data
    ...
    def load_by_path(self, path) -> np.ndarray:
        list=[]
        for t in range(len(path)):
            list.append(np.load(path[t]))
        data_array = np.array(list)
        return data_array

    def load_by_path_for_env(self, path) -> list:
        list=[]
        for t in range(len(path)):
            list.append(np.load(path[t], allow_pickle=True).item())
        return list
    ...
    def get(self, tr_scene, state, padding=np.nan) -> np.ndarray:
        ...
        # if (state=='gph_data'):
        #     data_array = self.gph_data[tr[0]:tr[1] + 1]
        #     # print("!!!in gph_data:data_array.shape:", data_array.shape)
        #     padded_data_array = np.full((length, data_array.shape[1], data_array.shape[2]), fill_value=padding)
        #     padded_data_array[paddingl:length - paddingu] = data_array
        if(state=='env_data'):
            data_array = self.env_data[tr[0]:tr[1] + 1]
            my_dict = {}
            for i, item in enumerate(data_array):
                my_dict[i] = item
            padded_data_array = my_dict  #dict
        # elif(state=='gph_all_hpa_data'):
        #     data_array = self.gph_all_hpa_data[tr[0]:tr[1] + 1]
        #     padded_data_array = np.full((length, data_array.shape[1], data_array.shape[2], data_array.shape[3]), fill_value=padding)
        #     padded_data_array[paddingl:length - paddingu] = data_array
        # elif (state == 'sst_data'):
        #     data_array = self.sst_data[tr[0]:tr[1] + 1]
        #     padded_data_array = np.full((length, data_array.shape[1], data_array.shape[2]),
        #                                 fill_value=padding)
        #     padded_data_array[paddingl:length - paddingu] = data_array
        # elif (state == 'tcwv_data'):
        #     data_array = self.tcwv_data[tr[0]:tr[1] + 1]
        #     padded_data_array = np.full((length, data_array.shape[1], data_array.shape[2]),
        #                                 fill_value=padding)
        #     padded_data_array[paddingl:length - paddingu] = data_array
        # elif (state == 'temperature_data'):
        #     data_array = self.temperature_data[tr[0]:tr[1] + 1]
        #     padded_data_array = np.full((length, data_array.shape[1], data_array.shape[2], data_array.shape[3]),
        #                                 fill_value=padding)
        #     padded_data_array[paddingl:length - paddingu] = data_array
        elif (state == 'wind_data'):
            data_array = self.wind_data[tr[0]:tr[1] + 1]
            padded_data_array = np.full((length, data_array.shape[1], data_array.shape[2], data_array.shape[3], data_array.shape[4]),
                                        fill_value=padding)
            padded_data_array[paddingl:length - paddingu] = data_array
        elif (state == 'gph_all_hpa_data'):
            data_array = self.gph_all_hpa_data[tr[0]:tr[1] + 1]
            # data_array = data_array[:, 0:1, 0:100, 0:100]
            # data_array = NumpyDataset(self.gph_200_all_data[tr[0]:tr[1] + 1])
            padded_data_array = np.full((length, data_array.shape[1], data_array.shape[2], data_array.shape[3]),
                                        fill_value=padding)
            padded_data_array[paddingl:length - paddingu] = data_array #[T1,1,100,100]
        elif (state == 'q_data'):
            data_array = self.q_data[tr[0]:tr[1] + 1]
            padded_data_array = np.full((length, data_array.shape[1], data_array.shape[2], data_array.shape[3]),
                                        fill_value=padding)
            padded_data_array[paddingl:length - paddingu] = data_array
        elif (state == 't_data'):
            data_array = self.t_data[tr[0]:tr[1] + 1]
            padded_data_array = np.full((length, data_array.shape[1], data_array.shape[2], data_array.shape[3]),
                                        fill_value=padding)
            padded_data_array[paddingl:length - paddingu] = data_array
        elif (state =='vo_cen'):
            data_array = self.vo_cen[tr[0]:tr[1] + 1]
            padded_data_array = np.full((length, data_array.shape[1], data_array.shape[2], data_array.shape[3]),
                                        fill_value=padding)
            padded_data_array[paddingl:length - paddingu] = data_array
        elif (state == 'vo_dis'):
            data_array = self.vo_dis[tr[0]:tr[1] + 1]
            padded_data_array = np.full((length, data_array.shape[1]),
                                        fill_value=padding)
            padded_data_array[paddingl:length - paddingu] = data_array
        # elif (state == 'gph_500_data'):
        #     data_array = self.gph_500_data[tr[0]:tr[1] + 1]
        #     padded_data_array = np.full((length, data_array.shape[1], data_array.shape[2]),
        #                                 fill_value=padding)
        #     padded_data_array[paddingl:length - paddingu] = data_array
        else:
